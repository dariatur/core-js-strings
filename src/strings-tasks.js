...
function removeFirstOccurrences(str, value) {
  if (str.includes(value)) {
    const substr1 = str.substring(0, str.indexOf(value));
    const substr2 = str.substring(
...
function removeLastOccurrences(str, value) {
  if (str.includes(value)) {
    const substr1 = str.substring(0, str.lastIndexOf(value));
    const substr2 = str.substring(
...
function formatTime(minutes, seconds) {
  const str1 = minutes.toString().padStart(2, '0').concat(':');
...
function countVowels(str) {
  ...
  for (let i = 0; i < newStr.length; i += 1) {
    if (vowels.includes(newStr.charAt(i))) {
      count += 1;
    }
  }
  return count;
...
function isPalindrome(str) {
  ...
  for (
    ...
    if (res[i].toLowerCase() !== res[j].toLowerCase()) {
      check = false;
    }
  }
  return check;
...
function encodeToRot13(str) {
  ...
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (input.includes(str.charAt(i))) {
      result += output.charAt(input.indexOf(str.charAt(i)));
    } else {
      result += str.charAt(i);
    }
  }
